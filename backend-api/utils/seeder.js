...
const seedData = async () => {
  try {
    ...
    try {
      ...
      console.log(
        "Dropped unique email index from consults collection successfully.",
      );
    } catch (err) {
    ...
    try {
      ...
      console.log(
        "Dropped unique mobile index from consults collection successfully.",
      );
    } catch (err) {
      if (err.codeName !== "IndexNotFound" && err.code !== 27) {
    ...
    if (!immunityCollection) {
      ...
      const sourceImg = path.join(
        __dirname,
        "../../public/shopbycolletions/immunitybooster.webp",
      );
    ...
    if (!kumbhprash) {
      ...
      kumbhprash = new Product({
        ...
        productDescription:
          "A premium Ayurvedic wellness formulation enriched with organic Kumbh honey, fresh Amla, and potent immunity-boosting herbs. It rejuvenates the body, enhances vitality, and strengthens the body's natural defense mechanism against seasonal infections.",
    ...
    let basant = await Product.findOne({
      productName: {
        $regex: /^(basant kusumakar ras|basnt kushumakar rash)$/i,
      },
    });
    ...
    if (!basant) {
      ...
      basant = new Product({
        ...
        productDescription:
          "A classical and highly potent Ayurvedic herbomineral medicine containing Swarna Bhasma (Gold), Rajat Bhasma (Silver), and Moti Bhasma (Pearl). Celebrated for its deep rejuvenating properties, it boosts immunity, enhances stamina, regulates sugar levels, and promotes overall longevity and vitality.",
