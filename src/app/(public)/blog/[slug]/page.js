...
import { FaCalendarAlt, FaUser, FaChevronRight, FaComments, FaArrowLeft, FaGlobe } from "react-icons/fa";
...
export default function BlogDetailsPage() {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      setFormError(err.response?.data?.message || "Failed to submit comment. Please try again.");
    } finally {
      ...
      <>
        ...
        <div className="bg-gray-50 min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 font-semibold">Preparing Ayurvedic Insights...</p>
          </div>
        </div>
        <Footer />
      </>
          ...
          <div className="max-w-7xl w-full mx-auto px-4 pb-12 relative z-10 text-white">
            ...
            <div className="mt-6 flex flex-wrap items-center gap-5 text-sm text-amber-200/90 font-medium">
              <span className="flex items-center gap-2">
                <FaUser className="text-amber-500 text-xs" /> By {blog.author || "Shree Baidyanath"}
              </span>
              <span className="hidden sm:inline text-amber-500/50">•</span>
              <span className="flex items-center gap-2">
                <FaCalendarAlt className="text-amber-500 text-xs" /> {formatDate(blog.createdAt)}
              </span>
              <span className="hidden sm:inline text-amber-500/50">•</span>
              <span className="flex items-center gap-2">
                <FaComments className="text-amber-500 text-xs" /> {comments.length} Reply/Replies
              </span>
            </div>
          </div>
        </div>
            ...
            <main className="lg:col-span-8 space-y-12">
              ...
              <section className="space-y-8 bg-white p-6 md:p-10 rounded-2xl border border-gray-100 shadow-sm">
                <h3 className="text-2xl font-bold text-gray-900 border-b border-gray-100 pb-4 flex items-center gap-2.5">
                  <FaComments className="text-amber-600" /> Comments ({comments.length})
                </h3>
                ...
                <div className="space-y-6">
                  {comments.length > 0 ? (
                    comments.map((cmt) => (
                      <div key={cmt._id} className="flex gap-4 p-4 rounded-xl hover:bg-gray-50/50 border border-gray-100/50 transition">
                        {/* Circular Avatar */}
                        ...
                        <div className="flex-1 space-y-1">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <span className="font-bold text-gray-900">{cmt.name}</span>
                          ...
                          {cmt.website && (
                            <a 
                              href={cmt.website.startsWith("http") ? cmt.website : `https://${cmt.website}`} 
                              target="_blank" 
                    ...
                    <div className="text-center py-6 text-gray-500 text-sm">
                      Be the first to share your thoughts on this Ayurvedic article!
                    </div>
                  )}
                </div>
                ...
                <div className="pt-6 border-t border-gray-100">
                  <h4 className="text-xl font-bold text-gray-900">Leave a Reply</h4>
                  <p className="text-gray-500 text-xs mt-1">Your email address will not be published. Required fields are marked *</p>

                  <form onSubmit={handleCommentSubmit} className="mt-6 space-y-5">
                    <div>
                      <label htmlFor="comment_body" className="block text-sm font-bold text-gray-700 mb-1">
                        Comment *
                      </label>
                      <textarea
                        id="comment_body"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label htmlFor="comment_name" className="block text-sm font-bold text-gray-700 mb-1">
                          Name *
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label htmlFor="comment_email" className="block text-sm font-bold text-gray-700 mb-1">
                          Email *
                        </label>
                        <input
                          type="email"
                    ...
                    <div>
                      <label htmlFor="comment_website" className="block text-sm font-bold text-gray-700 mb-1">
                        Website (Optional)
                      </label>
                      <input
                        type="url"
                  ...
                  {categories.map((cat, idx) => (
                    <li key={idx}>
                      <Link 
                        ...
                        <span className="flex items-center gap-2">
                          <FaChevronRight size={10} className="text-amber-500" />
                          {cat.name}
                        </span>
                  ...
                  {recentBlogs.length > 0 ? (
                    recentBlogs.map((item) => {
                      ...
                      return (
                        <div 
                          key={item._id}
                          onClick={() => navigateTo(router, `/blog/${item.slug}`)}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="flex items-center gap-1 text-[11px] text-gray-400 mt-1 font-medium">
                              <FaCalendarAlt size={9} /> {formatDate(item.createdAt)}
                            </span>
                          </div>
                        </div>
                      );
                    ...
                    <div className="text-xs text-gray-400">No other recent posts found.</div>
                  )}
                </div>
              </div>
