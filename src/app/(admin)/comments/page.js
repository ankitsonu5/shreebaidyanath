...
import { FaTrash, FaEye, FaSearch, FaEnvelope, FaTimes, FaCalendarAlt, FaBlog, FaGlobe } from "react-icons/fa";
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-2 sm:p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Blog Comment Moderation</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 rounded-xl border border-gray-100 shadow-sm text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-800 mb-1">No comments found</h3>
          <p className="text-gray-500 text-sm">
            {searchTerm ? "Try searching for a different keyword." : "Any submitted blog replies will appear here."}
          </p>
        </div>
      ) : (
                ...
                {filteredComments.map((item) => (
                  <tr key={item._id} className="hover:bg-gray-50/50 transition">
                    <td className="px-6 py-4 font-semibold text-gray-900">
                      <div className="max-w-[200px] truncate" title={item.blogId?.title || "Deleted Article"}>
                        {item.blogId?.title || <span className="text-red-500 italic">Deleted Article</span>}
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {item.website && (
                          <span className="flex items-center gap-1 text-xs text-gray-500">
                            <FaGlobe className="text-gray-400 text-[10px]" /> {item.website}
                          </span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <p className="truncate max-w-[300px] text-gray-600" title={item.comment}>
                        {item.comment}
                      </p>
                    </td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 animate-in zoom-in-95 duration-200">
            ...
            <div className="bg-amber-600 text-white p-5 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-amber-100 mt-0.5">Posted on {formatDate(selectedComment.createdAt)}</p>
              </div>
              <button
            ...
            <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider mb-1">Article Context</span>
                <span className="text-gray-900 font-bold flex items-center gap-2">
                  <FaBlog className="text-amber-600 text-sm" /> {selectedComment.blogId?.title || "Deleted Article"}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                <div>
                  <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider">Commenter Name</span>
                  <span className="text-gray-900 font-semibold">{selectedComment.name}</span>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider">Email Address</span>
                  <a href={`mailto:${selectedComment.email}`} className="text-amber-600 font-semibold hover:underline flex items-center gap-1.5 truncate">
                ...
                {selectedComment.website && (
                  <div className="col-span-2 border-t border-gray-200/50 pt-2.5 mt-1">
                    <span className="text-[10px] uppercase font-bold text-gray-400 block tracking-wider">Website</span>
                    <a href={selectedComment.website.startsWith("http") ? selectedComment.website : `https://${selectedComment.website}`} target="_blank" rel="noopener noreferrer" className="text-amber-600 font-semibold hover:underline flex items-center gap-1.5">
              ...
              {/* Message Statement */}
              <div>
                <span className="text-xs font-bold text-gray-500 uppercase block tracking-wider mb-2">Comment Message</span>
